                       ...
                       $query = "SELECT * FROM tblresident WHERE id='$id' ";
                        ...
                        if(mysqli_num_rows($query_run) > 0)
                        {
                            foreach($query_run as $row)
                            {
                                ...
                                <form action="function.php" method="POST" id="registration">
                                ...
                                <!-- END -->
                                    ...
                                    <p class="intro2"><b>ITO AY ISANG PAGPAPATUNAY</b> na si <b><?php echo $row['firstname']; ?><?php echo $row['middlename']; ?> <?php echo $row['lastname'];?></b> ay anak ni <b><?php echo $row['motherName']; ?></b> na ipinanganak sa Barangay na ito noong ika- <b><?php $birthdate = $row['birthdate']; $date = new DateTime($birthdate); $formattedDate = $date->format('j'); echo $formattedDate; ?> ng <?php $birthdate = $row['birthdate']; $date = new DateTime($birthdate); $formattedDate = $date->format('F'); echo $formattedDate; ?> ng taong <?php $birthdate = $row['birthdate']; $date = new DateTime($birthdate); $formattedDate = $date->format('Y'); echo $formattedDate; ?>.</b><br></p>   
                                    ...
                                    <p class="intro">Iginawad ngayong ika- <b><?php date_default_timezone_set('Asia/Tokyo'); $currentDateTime = date('j'); echo $currentDateTime;?> ng <?php date_default_timezone_set('Asia/Tokyo'); $currentDateTime = date('F Y'); echo $currentDateTime;?>,</b> dito sa Tanggapan ng Punong Barangay <?php echo ucwords(strtolower($row['barangay'])); ?>, <?php echo ucwords(strtolower($row['city'])); ?>, <?php echo ucwords(strtolower($row['province'])); ?>.</p></br></br></br></br>
                                <?php
                            }
                        }
                        else
...
<br>
    ...
    <button class="noprint" id="printpagebutton" form="registration" name="insert_registration" onclick="PrintElem('#clearance')" style="position: fixed">Print</button>
