                    ...
                    if(isset($_GET['id']))
                    {
                        ...
                        if(mysqli_num_rows($query_run) > 0)
                        {
                            foreach($query_run as $row)
                            {
                                ...
                                <form action="function.php" method="POST" id="closure">
                                ...
                                <!-- END -->
                                    ...
                                    <p class="intro"><b>THIS IS TO CERTIFY</b> that <b><?php echo $row['BusinessName']; ?></b> owned by <b><?php echo $row['firstname']; ?> <?php echo $row['middlename']; ?> <?php echo $row['lastname'];?></b>, resident of open and started its business operation on <b><?php $birthdate = $row['BusinessDateStarted']; $date = new DateTime($birthdate); $formattedDate = $date->format('F j, Y'); echo $formattedDate; ?></b> at <b><?php echo $row['BusinessPurokNo']; ?></b> of this Barangay had closed and stop its business operation on <b><?php date_default_timezone_set('Asia/Tokyo'); $currentDateTime = date('F j, Y'); echo $currentDateTime;?>.</b><br></p>
...
<br>
    ...
    <button class="noprint" id="printpagebutton" form="closure" name="insert_business_closure" onclick="PrintElem('#clearance')" style="position: fixed">Print</button>
