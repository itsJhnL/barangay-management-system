                    ...
                    if(isset($_GET['id']))
                    {
                        ...
                        if(mysqli_num_rows($query_run) > 0)
                        {
                            foreach($query_run as $row)
                            {
                                ...
                                <form action="function.php" method="POST" id="permit">
                                ...
                                <!-- END -->
                                    ...
                                    <p class="intro"><b>THIS IS TO CERTIFY</b> that <b><?php echo $row['firstname']; ?> <?php echo $row['middlename']; ?> <?php echo $row['lastname'];?>,</b> a resident of <b>PUROK <?php echo $row['purokNo'];?></b> of this Barangay had applied and been granted a <b>BARANGAY BUILDING PERMIT</b> as a requirement prior to the construction of the proposed building applied for.<br></p>
                                    ...
                                    <p class="intro">Issued this <b><?php date_default_timezone_set('Asia/Tokyo'); $currentDateTime = date('j'); echo $currentDateTime;?> day of <?php date_default_timezone_set('Asia/Tokyo'); $currentDateTime = date('F Y,'); echo $currentDateTime;?></b> hereat the Office of the Punong Barangay of Barangay <?php echo ucwords(strtolower($row['barangay'])); ?>, <?php echo ucwords(strtolower($row['city'])); ?>, <?php echo ucwords(strtolower($row['province'])); ?>.</p></br></br></br></br>
...
<br>
    ...
    <button class="noprint" id="printpagebutton" form="permit" name="insert_permit" onclick="PrintElem('#clearance')" style="position: fixed">Print</button>
